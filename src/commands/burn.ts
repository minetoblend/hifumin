...
export class BurnCommand extends Command {
	public override registerApplicationCommands(registry: Command.Registry) {
		registry.registerChatInputCommand((builder) =>
			builder
				...
				.addStringOption((option) => option.setName('card').setDescription('The card to burn').setRequired(false))
		);
	}
	...
	override async chatInputRun(interaction: ChatInputCommandInteraction) {
		...
		switch (await CardService.getCardUsage(db.createEntityManager(), card.id)) {
			case 'job_slot': {
				await response.edit({
					content: `Cannot burn ${card.username}, it is currently assigned to a job`
				});
				return;
			}
			default: {
		...
		const attachment = new AttachmentBuilder(imagePath).setName('card.png');
		...
		await response.edit({
			...
			components: [
				new ActionRowBuilder<ButtonBuilder>().addComponents(
					new ButtonBuilder().setCustomId(`burn:confirm:${card.id}`).setLabel('Confirm').setStyle(ButtonStyle.Primary),
					new ButtonBuilder().setCustomId(`burn:cancel:${card.id}`).setLabel('Cancel').setStyle(ButtonStyle.Danger)
				)
			]
		});
