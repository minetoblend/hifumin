...
export const GIF = function () {
  ...
  function processFrame(frame) { // creates a RGBA canvas image from the indexed pixel data.
      ...
      frame.image        =  createCanvas(gif.width, gif.height)
  ...
  function loadGif(filename) { // starts the load
      fetch(filename).then(response => response.arrayBuffer()).then(dataLoaded).catch(error);
      this.src = filename;
      this.loading = true;
  }
  ...
  var gif = {                      // the gif image object
      onload         : null as (() => void) | null,       // fire on load. Use waitTillDone = true to have load fire at end or false to fire on first frame
      onerror        : null as ((err: any) => void) | null,       // fires on error
